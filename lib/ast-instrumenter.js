...
function instrumentCodeWithAST(source, filename, options = {}) {
    try {
        ...
        traverse(ast, {
            ...
            // Handle callback arguments: createRoutine('TYPE', (payload) => {...})
            // This catches anonymous arrow/function expressions passed as arguments
            // to any function call — the key missing piece for HOF patterns.
            CallExpression(path) {
                const calleeName = getCalleeName(path.node.callee);
                if (!calleeName) return;

                path.node.arguments.forEach((arg, argIndex) => {
                    const isArrow = t.isArrowFunctionExpression(arg);
                    const isFuncExpr = t.isFunctionExpression(arg);
                    if (!isArrow && !isFuncExpr) return;

                    // Skip if this argument was already instrumented (e.g. a named
                    // function expression that another visitor already handled, or
                    // if the same CallExpression is visited twice due to AST rewriting).
                    const key = `${calleeName}_arg${argIndex}_${arg.start}`;
                    if (instrumentedFunctions.has(key)) return;
                    instrumentedFunctions.add(key);

                    const funcName = `${calleeName}_arg${argIndex}`;
                    const argPath = path.get(`arguments.${argIndex}`);

                    if (isArrow) {
                        instrumentInlineArrowFunction(argPath, funcName, filename, calleeName, argIndex, isTypeScript);
                    } else {
                        instrumentFunction(argPath, funcName, filename, 'callback_argument', isTypeScript);
                    }
                });
            }
        });
...
// Extract a human-readable name from a call expression callee node.
// Handles: foo(), foo.bar(), foo.bar.baz(), foo['bar']()
function getCalleeName(callee) {
    ...
    if (t.isMemberExpression(callee)) {
        ...
        const propName = callee.computed
            ? null // skip dynamic property access like foo[expr]()
            : (t.isIdentifier(callee.property) ? callee.property.name : null);
        if (objectName && propName) {
            return `${objectName}.${propName}`;
        }
        // If only property is available (e.g. this.foo()), use just the property
        return propName || objectName || null;
    }
    // For chained calls like foo()() — use inner call's callee name
...
// Instrument an inline arrow function that is passed as an argument to a call.
// Similar to instrumentArrowFunction but the path points directly at the
// ArrowFunctionExpression node (not a VariableDeclarator wrapping it).
function instrumentInlineArrowFunction(path, funcName, filename, calleeName, argIndex, isTypeScript) {
    ...
    if (instrumentationStatements.length === 0) return;
